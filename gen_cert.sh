...
OPTIONS:
    ...
    --download-ca
        Download the CA certificate and key released on github.
    --download-ca-version <DOWNLOAD_CA_VERSION>
        Specify the version of downloaded CA certificate and key. [env: DOWNLOAD_CA_VERSION=]
...
if [ -n "$DOWNLOAD_CA" ] || [ -n "$DOWNLOAD_CA_VERSION" ]; then
    if [ -f "$CA_KEY" ] || [ -f "$CA_CERT" ]; then
        echoerr "File \"$CA_KEY\" or \"$CA_CERT\" already exists. Please remove them or specify different names."
        exit 1
    fi
    curl -#fL "https://github.com/jay-kam/ssl-tool/releases/download/${DOWNLOAD_CA_VERSION:-v0.0.0}/ca.crt" -o $CA_CERT
    curl -#fL "https://github.com/jay-kam/ssl-tool/releases/download/${DOWNLOAD_CA_VERSION:-v0.0.0}/ca.key" -o $CA_KEY
fi
